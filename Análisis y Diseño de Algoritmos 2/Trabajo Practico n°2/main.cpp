...
bool EstrategiaPoda(list<Punto> Puntos,int x,int y,int Energia,int Costo)
{
    Punto Nuevo(x,y);
...
bool Poda(list<Punto> Puntos,int x,int y,int Energia)
{
    Punto Nuevo(x,y);
    list<Punto>::iterator it=Puntos.begin();
    for(it ; it != Puntos.end() ; it ++){
...
int main()
{
    ...
    MostrarMatriz(Matriz,Ancho,Alto);
